  ...
  return (
    <div className="flex flex-col md:flex-row justify-center items-start">
      <div className="md:w-1/2 p-4 justify-center ">
        <button
          className="bg-[#1976D2] hover:bg-[#1976d2e2] text-white p-3 rounded mb-5"
          onClick={handleBack}
        >
          <IoArrowBack />
        </button>
        ...
        {results.posts.length === 0 && (
          <div className="text-left mt-8">
            ...
            <p className="text-[#1976D2] text-xl underline underline-offset-3 font-bold cursor-pointer">
        ...
        {results.users.length > 0 && (
          <div className="mb-4 justify-center">
            ...
            {results.users.map((user) => (
              <div
                key={user._id}
                className="mb-3 border-2 border-slate-600 p-3 rounded"
              >
                <div className="flex justify-between items-center">
                  <div className="relative flex rounded-full bg-[#E8E8E8] h-10 w-10">
                    <img
                      ...
                      alt={`user-${user._id}`}
                    />
                    <p className="ml-4 text-white justify-center text-2xl font-bold">
                      {user.username}
                    </p>
                    <span>
                      {user.isVip && (
                        <MdOutlineVerified className="ml-2 mt-2 text-xl text-yellow-500" />
                      )}
                    </span>
                  </div>
                  <Link to={`/user/${user.username}`}>
                    <span className="flex items-center text-[#1976D2] hover:text-[#1976d2e2] hover:underline justify-center cursor-pointer text-2xl font-bold">
                      View
                    </span>
                  </Link>
                </div>
