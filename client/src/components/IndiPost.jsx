...
const IndiPost = ({ id, currentUser }) => {
  ...
  const checkIfUpvoted = (postData) => {
    ...
    if (
      upvotedPostsFromStorage &&
      upvotedPostsFromStorage.includes(postData._id)
    ) {
      setUpvoted(true);
    }
  };
        ...
        post && (
          ...
          >
            <div className="flex items-center mb-2 cursor-pointer">
              ...
              <a>
                <Link to={`/user/${post.posterUsername}`}>
                  <span className="text-blue-500 hover:text-[#1976d2e2] hover:underline text-2xl font-bold md:text-lg ">
                    {post.posterUsername}
                  </span>
                </Link>
              </a>
            ...
            <ReactMarkdown className="text-white text-2xl mb-2 font-bold">
              {post.title}
            </ReactMarkdown>
            <ReactMarkdown className="text-white mb-2 text-lg ">
              {post.content}
            </ReactMarkdown>
              ...
              {upvoted ? (
                <button className="flex items-center text-white">
                  <FaRegArrowAltCircleUp
                    className="mr-2.5 text-white hover:text-[#1976d2e2]"
                    onClick={removeUpvote}
                  />{" "}
                  {formatScore(post.upvotes)}
                </button>
              ) : (
                <button
